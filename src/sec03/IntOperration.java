...
public class IntOperration {

	public static void main(String[] args) {
		// 정수에서 int 이하 피연산차 값의 연산은 int 타입으로 변환되어 연산된다.

		byte b = 10;
		short s = 20;
		int add = b + s; // 첨파일 오류 (양쪽모두 int 타입으로 바뀌어서 연산.결과도 int 타입)
		add = (short) (b + s); // (short)타입으로 변환 했기 떄문에 가능

		// byte type 오버플로
		// byte b = 128; // byte type으로 강제 형변환해야(-128)
		System.out.println("add : " + add);

		// 코드 타이핑과 동시에 미리 연산을 하여 b2에 저장하라고 한다
		// 변수가 아니라 단순 리터형 값의 연산이므로 형변환 없다
		// 단지 왼쪽 변수 타입의 범위 내인지 만 체크
		byte b2 = 126 + 1; // byte 타입 범위 : -128 ~+127이므로 가능
		// b2 =128 + 2; // [오류]+128이 되어 오버플로우

		// 정수 변수 간의 연산은 기본적으로 int 타입으로 형변환 된 후의 연산되고 그 결과도 int 타입
		// int 타입간의 연산으로 결과가 int형으로 byte 변수 b3에 넣을 수 없음.
		byte b1 = 10;
		byte b3 = (byte) (b1 + 1);// [오류]add cast 해줘야함(값 손실 우려)
		System.out.println(b3);

		// int type 가질수 있는 최대값(2347483647)
		...
		int i = 1;
		i = 2147483647 + 1;// 결과는 2347483647이 되어 int 볌위를 넘어서지만
		System.out.println("i:" + i);// =2348493648
		...
		//longVal = 2147483648; //L/i : int type 읽지 말고 long type으로 읽어줘
		
		//정수 실수간 연산(float type으로 변환후 연산)
		float fVal = 100.2f; //대부분 더블타입으로 받기떄문에 float 타입인걸 알려주기 위해 f 쓴다 
		...
		//정수간 나눗셈의 결과는 정수이다.//소숨점 이하는 버려진다 . 
		int num1 = 9;
		int num2 = 2;
